Welcome to your demo of OhMyPCAP on Kubernetes!

### About

OhMyPCAP allows you to download a pcap file from the web or upload a pcap file from your local collection. It will then analyze that pcap and allow you to view security alerts, browse network metadata (DNS, HTTP, TLS, flows), extract ASCII transcripts, and carve individual streams — all from a single-page UI.

### Notes

Before we get started, please note the following:
- this is a cloud based service so please do not share any sensitive pcap files or any other sensitive info
- free accounts are limited to 60 minutes of usage before the instance is automatically terminated
- if you need a private or permanent instance of OhMyPCAP, please see the other options at https://github.com/dougburks/ohmypcap

### Get Started

To get started, sign into Killercoda using one of the options on the right and then click the START button below! ⬇
